...
import { Button } from "@/components/ui/button"
import { ArrowLeft, Download, Mic, FileEdit, Lightbulb, AlertCircle, Info } from "lucide-react"
import { useRouter, useSearchParams } from "next/navigation"
import { toast } from "@/components/ui/use-toast"
import { useRef } from "react"
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog"
...
const generateAISuggestions = async (data: ResumeData): Promise<Suggestion[]> => {
  try {
    // Detect profession from resume data
    const profession = detectProfessionFromData(data)

    const prompt = `You are an expert resume consultant. Analyze this resume data and provide personalized suggestions for improvement.
...
Please analyze the resume and provide suggestions in the following categories:
1. Critical issues (missing essential information)
2. Important improvements (significant enhancements)
3. General improvements (nice-to-have additions)

For each suggestion, consider:
- Industry-specific requirements for ${profession}
- Current job market trends
- ATS (Applicant Tracking System) optimization
- Professional presentation standards

Also provide an additional array of specific suggestions for what to include in the resume for a ${profession} role.
...
Return your response as a JSON object with this exact structure:
{
  "suggestions": [
    {
      "id": "unique-id",
      "type": "critical|important|improvement",
      "category": "contact|content|structure|keywords",
  ...
  "additionalSuggestions": [
    "Specific suggestion 1 for ${profession}",
    "Specific suggestion 2 for ${profession}",
    "Specific suggestion 3 for ${profession}",
    "Specific suggestion 4 for ${profession}",
    "Specific suggestion 5 for ${profession}"
  ]
}

Focus on actionable, specific advice. Limit to 8-12 suggestions total in the main suggestions array, and 5-8 items in additionalSuggestions.`
    ...
    console.log("[v0] AI-generated suggestions:", result.suggestions)
    ...
    return [
      {
        ...
        description: "Unable to generate personalized suggestions at this time.",
        action: "Please review your resume manually for completeness and accuracy.",
      },
    ]
  }
}
...
export default function ResumePreview() {
  ...
  const categoryId = searchParams.get("category") || ""
  const resumeId = searchParams.get("id") || ""
  const selectedLanguage = (searchParams.get("lang") || "hi") as Language
  ...
  useEffect(() => {
    if (resumeDataEnglish) {
      const loadSuggestions = async () => {
        ...
        try {
          // Detect profession from resume data
          const profession = detectProfessionFromData(resumeDataEnglish)

          const prompt = `You are an expert resume consultant. Analyze this resume data and provide personalized suggestions for improvement.

Resume Data:
${JSON.stringify(resumeDataEnglish, null, 2)}
...
Focus on actionable, specific advice. Limit to 8-12 suggestions total in the main suggestions array, and 5-8 items in additionalSuggestions.`
  ...
  const renderTemplate = (resumeData: ResumeData | null, language: Language = "en") => {
    if (!resumeData) return null
    ...
    if (effectiveTemplateId.startsWith("d")) {
      const variant = effectiveTemplateId.substring(1)
      return <DriverTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("dp")) {
      const variant = effectiveTemplateId.substring(2)
      return <DeliveryPartnerTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("e") && effectiveTemplateId.length === 2) {
      const variant = effectiveTemplateId.substring(1)
      return <ElectricianTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("t")) {
      const variant = effectiveTemplateId.substring(1)
      return <TailorTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("c") && effectiveTemplateId.length === 2) {
      const variant = effectiveTemplateId.substring(1)
      return <CookChefTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("s") && effectiveTemplateId.length === 2) {
      const variant = effectiveTemplateId.substring(1)
      return <SecurityGuardTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("sa")) {
      const variant = effectiveTemplateId.substring(2)
      return <SalesAssistantTemplate variant={variant} data={resumeData} language={language} />
    } else if (effectiveTemplateId.startsWith("m") && effectiveTemplateId.length === 2) {
      const variant = effectiveTemplateId.substring(1)
      return <MechanicTemplate variant={variant} data={resumeData} language={language} />
    } else {
      return <DriverTemplate variant="1" data={resumeData} language={language} />
    }
  }
  ...
  return (
    <div className="p-4">
      ...
      {isLoading ? (
        ...
        <div>
          {/* Side-by-side display when both English and translated versions exist */}
          ...
          {resumeDataOther ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-semibold">English Version</h2>
                ...
                <div id="pdf-content" className="bg-white shadow-lg rounded-lg overflow-hidden">
                  {renderTemplate(resumeDataEnglish, "en")}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-semibold">
                    {selectedLanguage === "hi" ? "Hindi Version" : "Translated Version"}
                  </h2>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={handleDownloadPDF}>
                      <Download className="mr-2 h-4 w-4" />
                ...
                <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                  {renderTemplate(resumeDataOther, selectedLanguage)}
                </div>
              </div>
            </div>
          ) : (
            // Single resume display when only English version exists
            <div className="space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div id="pdf-content" className="bg-white shadow-lg rounded-lg overflow-hidden max-w-4xl mx-auto">
                {renderTemplate(resumeDataEnglish, "en")}
              </div>
            </div>
          )}
