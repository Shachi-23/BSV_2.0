...
#     """Get templates filtered by profession category"""
#     category_mapping = {
#         'driver': ['p1', 'p2', 'm1', 'm2'],
#         'teacher': ['p1', 'p3', 'p4', 'c1'],
#         'engineer': ['p2', 'p4', 'p5', 'c2'],
#         'healthcare': ['p1', 'p3', 'e1', 'e2'],
#         'business': ['p2', 'p4', 'e1', 'e3'],
#         'creative': ['c1', 'c2', 'c3', 'c4', 'c5'],
#         'executive': ['e1', 'e2', 'e3', 'e4', 'e5']
#     }
    
#     template_ids = category_mapping.get(category.lower(), list(TEMPLATES.keys()))
...
#         prefix = profession_to_prefix.get(profession)
#         if not prefix:
#             return jsonify({'error': 'Invalid profession'}), 400
            
#         # Filter templates by profession prefix
...
#             'templates': profession_templates
#         })
...
def create_profession_specific_prompt(transcript, profession, language):
    ...
    prompt = f"""
You are an expert resume writer specializing in {profession} positions. 
...
TASK: Convert this transcript into a structured resume JSON format optimized for {profession} positions.

FOCUS ON THESE KEY AREAS for {profession}:
{', '.join(template['focus_areas'])}

REQUIREMENTS:
1. Extract and structure all relevant information from the transcript
2. Optimize content specifically for {profession} roles
3. Provide the response in BOTH English and {language}
4. Include suggestions for additional content the user should consider adding
...
{{
  "english": {{
    "personalInfo": {{
      ...
      "email": "extracted email or 'your.email@example.com'",
      "phone": "extracted phone or '+91 XXXXXXXXXX'",
      "address": "extracted address or 'Your Address'"
    }},
    "professionalSummary": "2-3 sentence summary optimized for {profession}",
    "experience": [
      {{
        "jobTitle": "job title",
  ...
  "{language}": {{
    "personalInfo": {{
      "name": "translated name or appropriate placeholder",
      "email": "same email",
      "phone": "same phone", 
      "address": "translated address or appropriate placeholder"
    }},
    "professionalSummary": "translated professional summary",
    "experience": [
      {{
        "jobTitle": "translated job title",
        "company": "company name (keep original if proper noun)",
        "duration": "translated duration format",
        "responsibilities": ["translated responsibility 1", "translated responsibility 2"]
      }}
    ],
    ...
    "certifications": ["translated certification1"],
    "achievements": ["translated achievement1"]
  }},
  "suggestions": {{
    "english": {{
      "title": "Suggestions to Strengthen Your {profession} Resume",
      "items": {template['suggestions']}
    }},
    "{language}": {{
      "title": "translated suggestions title",
      "items": ["translated suggestion1", "translated suggestion2", "translated suggestion3"]
    }}
  }}
}}

IMPORTANT: 
- If information is missing from transcript, use appropriate placeholders
- Focus on {profession}-specific terminology and skills
- Ensure translations are accurate and culturally appropriate
- Keep proper nouns (company names, places) in original language where appropriate
- Return ONLY the JSON, no additional text
"""
    
    return prompt

def convert_text_to_json_enhanced(transcript, profession="driver", language="en"):
    """Enhanced function to convert text to JSON with profession-specific optimization and multilingual support"""
    
    # Create profession-specific prompt
    prompt = create_profession_specific_prompt(transcript, profession, language)
    
    # Call OpenRouter API
    response = call_openrouter_api(prompt, model="openai/gpt-4o-mini")
    ...
    try:
        ...
        print(f"Error parsing OpenRouter JSON response: {e}")
...
def process_audio():
    ...
    language = request.form.get('language', 'en')
    profession = request.form.get('profession', 'driver')  # Get profession from request
    ...
    try:
        ...
        response_json = convert_text_to_json_enhanced(transcript, profession, language)
